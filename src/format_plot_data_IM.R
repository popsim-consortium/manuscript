...
setwd("~/Desktop/PopSim/analysis/IM/generic_IM_whole_genome/")
...
dadi$Na <- dadi$theta/(4*1e-8*simulated_genome_length)
...
truth_y_af <- rep(x=N_A, times = -(t_start-t_div)/step_size_truth )
truth_y_af <- c(truth_y_af, rep(x=N_2, times = -t_div/step_size_truth+1))
...
truth_y_eu <- rep(x=N_A, times = -(t_start-t_div)/step_size_truth )
truth_y_eu <- c(truth_y_eu, rep(x=N_1, times = -t_div/step_size_truth+1))
...
write.csv(plot_df_eu_3,"IM_popsize_pop1.csv", row.names=FALSE)
write.csv(plot_df_af_3,"IM_popsize_pop2.csv", row.names=FALSE)



### make data frames of divergence times
dadi_tdiv = data.frame(c(dadi$TDIV[1:3],t_div ), c("rep1","rep2", "rep3", "truth"), c("dadi","dadi","dadi","dadi"))
colnames(dadi_tdiv) <- c("x","rep", "method")

fsc_tdiv = data.frame(c(fsc$TDIV[1:3],t_div ), c("rep1","rep2", "rep3", "truth"), c("fsc","fsc","fsc","fsc"))
colnames(fsc_tdiv) <- c("x","rep", "method")

smcpp_tdiv = data.frame(c(max(smc1_eu$x), max(smc2_eu$x), max(smc3_eu$x),t_div ), c("rep1","rep2", "rep3", "truth"), c("smcpp","smcpp","smcpp","smcpp"))
...
setwd("~/Desktop/PopSim/figures/data/")
write.csv(div_times, "IM_tdiv.csv", row.names=FALSE)
