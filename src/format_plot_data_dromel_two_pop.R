...
setwd("~/Desktop/PopSim/analysis/two_population_analysis/dmel_n50_LSTP_joint_3Mb_no_mask/")


######## Read in dadi and fsc files ##########
simulated_genome_length <- 3000000 # genome length for 3 Mb dmel sims
dadi <- read.table("Results/best_dadi_runs.txt", header=T)
dadi$Na <- dadi$theta/(4*8.4e-9*simulated_genome_length)
dadi$N1 <- dadi$nu1*dadi$Na
dadi$N2 <- dadi$nu2*dadi$Na
dadi$TDIV <- dadi$T*dadi$Na*0.1*2
...
### this script is hard coded to plot three replicates - could change to up to 10
num_reps <- 3
...
generation_time=0.1
N_A0 = 8.603e06
t_A0 = 600000*generation_time  # assuming 10 generations / year
N_A1 = N_A0 / 5.0

# European Parameter values from "Demo History of Euro Population"
N_E0 = 1.075e06
N_E1 = 2200
t_AE = 158000*generation_time
t_E1 = t_AE - 3400*generation_time
...
truth_y_af <- rep(x=N_A1, times = -(t_start-t_A0)/step_size_truth )
truth_y_af <- c(truth_y_af, rep(x=N_A0, times = -t_A0/step_size_truth+1))
...
### census size for eu pop

truth_x_eu <- seq(from=t_start, to = 0, by = step_size_truth)
truth_y_eu <- rep(x=N_A1, times = -(t_start-t_A0)/step_size_truth )

truth_y_eu <- c(truth_y_eu, rep(x=N_A0, times = -(t_A0-t_AE)/step_size_truth))
truth_y_eu <- c(truth_y_eu, rep(x=N_E1, times = -(t_AE-t_E1)/step_size_truth))
truth_y_eu <- c(truth_y_eu, rep(x=N_E0, times = -t_E1/step_size_truth+2))
...
write.csv(plot_df_eu_3,"dromel_popsize_europe.csv", row.names=FALSE)
write.csv(plot_df_af_3,"dromel_popsize_africa.csv", row.names=FALSE)



### make data frames of divergence times
dadi_tdiv = data.frame(c(dadi$TDIV[1:3],t_AE ), c("rep1","rep2", "rep3", "truth"), c("dadi","dadi","dadi","dadi"))
colnames(dadi_tdiv) <- c("x","rep", "method")

fsc_tdiv = data.frame(c(fsc$TDIV[1:3],t_AE ), c("rep1","rep2", "rep3", "truth"), c("fsc","fsc","fsc","fsc"))
colnames(fsc_tdiv) <- c("x","rep", "method")

smcpp_tdiv = data.frame(c(max(smc1_eu$x), max(smc2_eu$x), max(smc3_eu$x),t_AE ), c("rep1","rep2", "rep3", "truth"), c("smcpp","smcpp","smcpp","smcpp"))
...
setwd("~/Desktop/PopSim/figures/")
write.csv(div_times, "dromel_tdiv.csv", row.names=FALSE)
