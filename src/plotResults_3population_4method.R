...
## define variables
infile_pop1="/n_t/homo_sapiens_mask_Ragsdale_population1/Results/all_estimated_Ne.txt"
infile_pop2="/n_t/homo_sapiens_mask_Ragsdale_population2/Results/all_estimated_Ne.txt"
infile_pop3="/n_t/homo_sapiens_mask_Ragsdale_population3/Results/all_estimated_Ne.txt"
outfile="/n_t/n_t_plot_Ragsdale.pdf"
...
tab0 <- read.table(file=infile_pop1, h=T ,sep="\t")
tab1 <- read.table(file=infile_pop2, h=T ,sep="\t")
tab2 <- read.table(file=infile_pop3, h=T ,sep="\t")
...
speciesPop <- bquote(italic(.(species)) ~ (.(pop1_name)))
p0A<-ggplot(tab0A, aes(x=x, y=y, group=interaction(rep), colour=rep)) +
  ...
  labs(x="", y="", title=speciesPop, subtitle=method1_name, colour="", fill="")+
...
p1A<-ggplot(tab1A, aes(x=x, y=y, group=interaction(rep), colour=rep)) +
  ...
  geom_step(data=tab1Z,aes(x=x, y=y),size=2.5,alpha=1.0, col=truth_color) +
  geom_step(size=1.2, alpha=0.9, show.legend = FALSE) +
  scale_x_continuous(trans='log10') +
  scale_y_continuous(trans='log10') +
  annotation_logticks(sides="b") +
...
pg<-plot_grid(p0A, p0B, p0C, p0D, p1A, p1B, p1C, p1D, p2A, p2B, p2C, p2D, ncol=4, rel_widths = c(1.3,1,1,1), rel_heights = c(1.05,1,1.15))
