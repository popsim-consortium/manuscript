...
a uniform recombination rate (Figure \ref{fig:slim_val_nomap}), likely due to the lower variation in
...
in which linkage disequilibrium is reduced (Figures~\ref{fig:n_t_sheehan} and \ref{fig:n_t_AraTha}).
...
In addition we would like to thank NESCENT and Matt Hahn, Victoria Sork, and Michael Whitlock for organizing a 2014
catalysis meeting in which many of the goals of this effort were first laid out. 
...
of human chromosome 22 under the AncientEurasia\_9K19 model \citep{kamm2019efficiently} using the
HapMapII\_GRCh37 genetic map \citep{international2007second}. 12 samples were drawn from each population 
(excluding basal Eurasians). From top to bottom we show results
using three scaling factors for the population sizes: Q=1, Q=10, and Q=50.
Kolmogorov-Smirnov 2-sample test statistics (\textit{D}) and p-values are shown, testing the null hypothesis that
the quantiles were drawn from the same continuous distribution.
}
...
population genetic summary statistics: \textit{r}\textsuperscript{2}, Tajima's $\pi$, and Tajima's D. Additionally, we
show runtimes for generating each simulation replicate. Data were generated by simulating 100 replicates
of human chromosome 22 under the AncientEurasia\_9K19 model \citep{kamm2019efficiently} using a 
uniform rate of recombination across the chromosome. 12 samples were drawn from each population 
(excluding basal Eurasians). From top to bottom we show results
using three scaling factors for the population sizes: Q=1, Q=10, and Q=50.
Kolmogorov-Smirnov 2-sample test statistics (\textit{D}) and p-values are shown, testing the null hypothesis that
the quantiles were drawn from the same continuous distribution.
