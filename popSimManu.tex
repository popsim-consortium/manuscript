...
Our goal for the two population inference pipeline was to explore two population inference
...
well as the \emph{Drosophila} Li & Stephan model (\cite{li2006inferring}). We implemented three
commonly-used multi-population inference approaches: \dadi (\cite{gutenkunst2009inferring}),
\fastsimcoal2 (\cite{excoffier2013robust}), and \smcpp (\cite{terhorst2017robust}). Our aim was not
to exhaustively test the performance of these methods, but rather to explore their behavior
...
the ancestral population size, the size of population 1 after split, the size of
population 2 after split, the divergence time, and two migration rate parameters. Importantly,
this meant that for both the human and \emph{Drosophila} inference, the fitted model
did not match the simulated model (Figure \ref{fig:IM_popn_human}C), which, for humans,
includes three populations with various population size changes as well as exponential
growth. In the human case, we therefore performed inference solely on the Africa (YRI)
and Europe (CEU) populations, meaning that the Asia (CHB) population functioned as a ghost
population that was ignored by our inference. Our motivation for this simple
approach was to mimic the typical approach of two population inference on empirical
...
% CK: I have a figure of the generic IM results in the works that we can include in the supplement

To perform inference, we began by running simulations under each model, performing
...
samples each from the CEU and YRI populations. For \emph{Drosophila},
we simulated 3 Mb of chromosome 2R under the Li & Stephan model (\cite{li2006inferring}),
using the map inferred by \cite{comeron2012many} and taking 50 samples from each of
the Africa and Europe populations. Due to computational limitations, we were unable
to run whole genome simulations for \emph{Drosophila}. For the generic IM simulations,
we used the human genome (chromosomes 1-22) and HapMap II recombination map (\cite{international2007second})
and took 20 samples per population.
% CK: should we say more about the drosophila simulations here?
...
chromosomes in the appropriate format for \dadi and \fastsimcoal2.
For each simulation replicate, we performed 10 runs of \dadi and \fastsimcoal2 and
checked for convergence. The parameter estimates from the top run from each simulation
replicate are shown in Figures \ref{fig:IM_popn_human}C and Sx.
% CK: how much detail should i go into here? should i include things such as initial
% parameter values, grid size, priors, etc?
...
human Gutenkunst model that does include migration. However, because \smcpp allows
for continuous population size changes, it is better equipped to capture many of the
more complex aspect of the simulated demographic models (e.g., exponential growth).
% CK: would be good to have someone with more smcpp experience to weigh in on
% these methods (Andy or Deigo?). Are there any settings that need to be mentioned?
