...
Currently, more models of human history have been implemented than for
any other organism (see Table \ref{tab:catalog}). 
...
in a number of the demographic models described above. The workflow was generalized to
run $R$ replicates with $C$ chromosomes for $n$ samples, for a total of $R \times C$
simulations for each demographic model. After simulation, the workfow prepares
...
run on whole genome simulations under under OutOfAfricaArchaicAdmixture\_5R19,
...
% ALG: I still think this sentence is confusing.
...
recover a modern population size within a factor of two of the truth, however
...
growth in the CEU population, though the inferred population sizes are in generally noisier. 
In addition, the assumption of no migration by \smcpp leads to divergence time
...
% redundant.
% ALG: I'm not sure about that. I don't really see that it's covered below.
...
% ALG: Mention in what way?

% ALG: I think the paragraph below has unnecessary detail that we don't need at this point.
...
of the model parameters and populations. 
% ALG: I’m not sure what documentation of populations means. 
This model is submitted on GitHub as a Pull Request,
...
Inclusion of a preliminary model 
% ALG: automatically?
triggers a QC issue, and a second developer,
...
If the two implementations are equal the original model is included in \stdpopsim.
...
For HomSap we ran replicate whole genome simulations under the OutOfAfrica\_3G09 model
...
% ALG: Is this also in the Snakefile?
...
% ALG: If you have Snakefile for the whole workflow, I would just say that at the end here.
