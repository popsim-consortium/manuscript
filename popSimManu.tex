...
% RNG: Fixed time and removed unexplained acronyms.
...
on a catalog of species (Fig. \ref{fig:cartoon}A), initially consisting of humans, \emph{D.~melanogaster}, \emph{A.~thaliana}, \emph{E.~coli}, and\emph{P.~abelii}. 
...
and we have already implemented \texttt{SLiM} \citep{haller2019slim} as
...
% RNG: Not competitors... We complement each other ;-)
...
run on whole genome simulations under an empirical genetic map and a model of human migration out of Africa that includes archaic admiture \citep{ragsdale2019models}. In each column of this figure
...
% RNG: Removed duplicated info from caption.
...
% RNG: Updated caption to reflect what is shown.
$N(t)$ trajectories for each replicate. In black we show the census population size from the simulated model.
...
% RNG: Added appropriate Jody Hey citation
with constant population sizes and bi-directional migration \citep{hey2004im}. For human
...
simulated under the \cite{gutenkunst2009inferring} model of human migration out of Africa.
Each of the methods compared has stronger
...
% RNG: Not in present manuscript.
%To ground-truth our inference approach, we also conducted inference on a generic IM model that was identical to the model used for inference.
